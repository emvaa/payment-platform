...
docker-compose exec postgres-payment psql -U payment_user -d payment_service -c "
...
echo "💰 Migrating Wallet Service database..."
docker-compose exec postgres-wallet psql -U wallet_user -d wallet_service -c "
CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\";
CREATE EXTENSION IF NOT EXISTS \"pgcrypto\";
...
echo "- Payment DB: docker-compose exec postgres-payment psql -U payment_user -d payment_service -c \"SELECT COUNT(*) FROM payments;\""
echo "- Wallet DB: docker-compose exec postgres-wallet psql -U wallet_user -d wallet_service -c \"SELECT COUNT(*) FROM users;\""
